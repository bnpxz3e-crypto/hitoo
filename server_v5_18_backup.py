            ...
            for gid, old in x['goods'].items():
                ...
                if isinstance(old, dict):
                    for lvl in ('SP', 'A'):
                        ...
                        for v in q.get('runs', []) if isinstance(q, dict) else []:
                            try:
                                runs.append(max(0, int(v)))
                            except:
                                pass

                        cabinet[lvl]['runs'] = runs[-100:]
...
def stats_summary(gid=None):
    ...
    if gid is None:
        ...
        for goods_id, cabinet in s.get('goods', {}).items():
            out = {}
            ...
            out['history'] = list(
                reversed(cabinet.get('history', []))
            )

            result[str(goods_id)] = out

        return {'goods': result}

    gid = str(gid)

    cabinet = s.get('goods', {}).get(
        gid,
        {
            'SP': {'current': 0, 'runs': []},
            'A': {'current': 0, 'runs': []},
...
def record_pack_stats(gid, pack_rows):
    ...
    for pack_no, rows in enumerate(pack_rows, 1):
        ...
        lvls = {
            str(x.get('shang_title') or '').upper()
            for x in rows
        }

        # SP/A ของตู้นี้เท่านั้น
        for target in ('SP', 'A'):
...
def local_card_shang_count(p):
    ...
    for lvl, sid, prefix in [
        ...
        # ส่งเฉพาะสถานะปัจจุบัน
        # ไม่ส่ง completed runs เก่า
        # จึงไม่มีแท่งสีน้ำเงินซ้ำ
        arr = [{
            'id': -sid,
            'user_id': 999999,
            'goodslist_id': 0,
            ...
            'interval_num': cur,
            'order_interval_num': now,

            'goodslist_price': '0.000',
            'shang_id': sid,
            'sale_num': 0,
            'addtime': now,
        ...
        data[prefix + 'not_yet_released'] = cur

        # เรทของ goods_id นี้
        data[prefix + '_probability'] = rate
        data[prefix + '_gailv'] = rate

        # สถิติของ goods_id นี้เท่านั้น
        data[prefix + '_all_probability'] = avg
        data[prefix + '_average'] = mx
...
def order(gid,n):
    ...
    for i in range(n):
      ...
      print(
          '[ORDER RESULT DEBUG]',
          'gid=', gid,
          'level=', x.get('shang_title'),
          'back_image=', x.get('back_image'),
          'color_t=', x.get('color_t'),
          'background=', x.get('background'),
          'imgurl_black=', x.get('imgurl_black')
      )
    ...
    return {
        ...
        'data':{
            'pay_type':1,
            'get_score':0,
            'order_num':f'LOCAL-{int(time.time())}',
            'sum_use_money':n*2,
...
def card_pool_for_gid(gid):
    ...
    # ==========================================
    # 1. HAR pool ใหม่ — ข้อมูลครบ SP/A/B/C/D
    # ==========================================
    rows = HAR_CARD_POOLS.get(gid)

    # ==========================================
    # 2. pool เดิม
    # ==========================================
    if not isinstance(rows, list) or not rows:
        rows = CARD_POOLS.get(gid)

    # ==========================================
    # 3. album เดิม
    # ==========================================
    if not isinstance(rows, list) or not rows:
        rows = CARD_ALBUM.get(gid)

    # ==========================================
    # 4. fallback จาก card/detail
    # ==========================================
    if not isinstance(rows, list) or not rows:
        detail = CARD_DETAILS.get(gid, {}).get("data", {})
        rows = detail.get("goodslist_all", [])
    ...
    for index, item in enumerate(rows, 1):
        ...
        try:
            sid = int(x.get("shang_id") or 0)
        except (TypeError, ValueError):
            sid = 0

        lvl = str(
            x.get("shang_title")
            or sid_to_level.get(sid, "")
        ).strip().upper()

        if lvl not in {"SP", "A", "B", "C", "D"}:
            continue
        ...
        # ------------------------------
        # Probability ของการ์ดใบนั้น
        # ------------------------------
        try:
            x["real_pro"] = float(x.get("real_pro") or 0)
        except (TypeError, ValueError):
            x["real_pro"] = 0.0

        # ------------------------------
        # Badge
        # captured detail บางตัวสะกด shang_iamge
        # ------------------------------
        badge = (
            x.get("shang_imgurl")
            or x.get("shang_image")
            or x.get("shang_iamge")
            or LEVEL_BADGES.get(lvl, "")
        )
...
def card_order(gid,n):
    ...
    # Card pool:
    # Album -> CARD_POOLS -> CARD_DETAILS.goodslist_all
    # ใช้ได้กับทุกตู้โดยอัตโนมัติ
    pool = card_pool_for_gid(gid)
    ...
    if not pool:
      return {
        ...
        # ============================================================
    # CARD DRAW V5.31
    # 1) สุ่ม Tier จากอัตราจริงของตู้
    # 2) สุ่มการ์ดจริงภายใน Tier จาก real_pro
    # 3) ห้ามตัด Tier ที่ไม่มีข้อมูลแล้ว normalize เงียบ ๆ
    # ============================================================
    ...
    for z in pool:
      ...
      lvl = str(
          z.get('shang_title') or ''
      ).strip().upper()

      if lvl in ('SP', 'A', 'B', 'C', 'D'):
        available.setdefault(lvl, []).append(z)

    # ลำดับ Tier ตายตัว
        # ============================================================
    # 5 ตู้ที่มี HAR ครบ ใช้ STRICT
    # ตู้อื่นใช้ระบบเดิม เพื่อไม่ให้ตู้เก่าพัง
    # ============================================================

    HAR_COMPLETE_GIDS = {"11", "13", "17", "22", "23"}
    strict_pool = str(gid) in HAR_COMPLETE_GIDS

    tier_order = ("SP", "A", "B", "C", "D")
    ...
    for lvl in tier_order:
      ...
      try:
        rate = float(tier_rates.get(lvl, 0) or 0)
      except:
        rate = 0.0
      ...
      if strict_pool:
        if not available.get(lvl):
          return {
            ...
            "msg": (
              f"Missing HAR cards: "
              f"goods_id={gid}, tier={lvl}, rate={rate}"
            ),
            "data": None
          }
      ...
      # ตู้เก่า: ใช้เฉพาะ Tier ที่ local มี
      else:
        if available.get(lvl):
          levels.append(lvl)
          weights.append(rate)

    # fallback เฉพาะตู้เก่า
    if not levels and not strict_pool:
      levels = [
        lvl
        for lvl in tier_order
        if available.get(lvl)
    ...
    if not levels:
      return {
        "status": 0,
        "msg": f"No usable local card prizes for goods_id {gid}",
        "data": None
      }
    ...
    print(
      "[CARD RATES]",
      "goods_id=", gid,
      "mode=", "HAR-STRICT" if strict_pool else "LEGACY",
      "rates=", {
        lvl: tier_rates.get(lvl, 0)
        for lvl in levels
      },
      "pool=", {
        lvl: len(available.get(lvl, []))
        for lvl in levels
      }
    )
    ...
    for i in range(n):
      ...
      # ==========================================
      # STEP 2: สุ่มการ์ดภายในระดับ
      # ==========================================
      item_weights = []

      has_real_probability = False

      for z in candidates:
        try:
          w = float(z.get('real_pro') or 0)
        except:
          w = 0.0

        if w > 0:
          has_real_probability = True

        item_weights.append(max(0.0, w))

      # ถ้า tier นี้มี real_pro จริง ใช้ตามนั้น
      if has_real_probability and sum(item_weights) > 0:
      ...
      # ถ้าไม่มี real_pro เลย จึงค่อย equal random
      else:
