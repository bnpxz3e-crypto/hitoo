...
def infinite_local_url(value):
    """Normalize JOYPOP image URLs to this local server."""
    if not value:
        return ''
    ...
    for prefix in ('https://img.joypop.gg/','http://img.joypop.gg/'):
        if value.startswith(prefix):
            return '/local-img/' + value[len(prefix):].lstrip('/')

    if '/local-img/' in value:
        return '/local-img/' + value.split('/local-img/',1)[1].lstrip('/')

    if value.startswith(('uploads/','upload/','static/','newimage/','images/','webSite/','badge/','card/')):
        return '/local-img/' + value.lstrip('/')
    if value.startswith(('/uploads/','/upload/','/static/','/newimage/','/images/','/webSite/','/badge/','/card/')):
        return '/local-img/' + value.lstrip('/')
    return value
...
def infinite_find_product_image(item):
    """Choose a product image only when its local file exists (or it is a real external URL)."""
    fields=(
        'goodslist_imgurl','imgurl','content_image','image','goods_imgurl',
        'goods_image','prize_image','prize_img','product_image','product_img',
        'cover','cover_image','thumb','thumbnail','imgurltwo'
    )
    candidates=[]
    for key in fields:
        raw=item.get(key)
        if not raw:
            continue
        url=infinite_local_url(raw)
        if url and url not in candidates:
            candidates.append(url)

    for url in candidates:
        if '/local-img/' in url and infinite_local_file_exists(url):
            return url
    for url in candidates:
        if url.startswith(('http://','https://')) and '/local-img/' not in url:
            return url
    return ''


def infinite_find_background(gid, level, item):
    """Find an Infinite result background, preferring captured assets that actually exist."""
    gid=str(gid)
    level=str(level or '').strip().upper()
        # Background แยกตามตู้
    CABINET_BACKGROUNDS = {
        # ตู้ 92 = พื้นน้ำแข็ง
        "92": "/local-img/newimage/202607/6ae99bb8feb158079cae09b9daad7730b956bd91.webp",
    }

    cabinet_bg = CABINET_BACKGROUNDS.get(gid)

    if cabinet_bg and infinite_local_file_exists(cabinet_bg):
        return infinite_local_url(cabinet_bg)
    pool=POOLS.get(gid,{}) or {}
    fields=(
        'back_image','background_image','background','goods_back_image',
        'goods_background','shang_back_image','result_background','result_back_image'
    )

    # Captured background on the selected prize.
    for key in fields:
        raw=item.get(key)
        if raw:
            bg=infinite_local_url(raw)
            if '/local-img/' not in bg or infinite_local_file_exists(bg):
                return bg

    # Captured background from another item in the same tier.
    tier_items=pool.get(level,[]) or []
    for source in tier_items:
        for key in fields:
            raw=source.get(key)
            if raw:
                bg=infinite_local_url(raw)
    ...
    # color_t only when the captured file is actually present.
    for source in [item] + list(tier_items):
        raw=source.get('color_t') if isinstance(source,dict) else None
        if raw:
            bg=infinite_local_url(raw)
            if '/local-img/' not in bg or infinite_local_file_exists(bg):
                return bg

    # Files confirmed present in this local mirror.
    tier_fallback={
        'SP':'/local-img/images/draw/SP_BG.png',
        'A':'/local-img/images/draw/A_BG.png',
    }
    fallback=tier_fallback.get(level,'')
    if fallback and infinite_local_file_exists(fallback):
        return fallback
    return ''
...
def apply_infinite_presentation(gid, item):
    ...
    if background:
        ...
        # background ของ Result
        item['back_image']=background

        # เพิ่มชื่อ field เผื่อหน้า frontend
        # แต่ละ version อ่านคนละชื่อ
    ...
    # สำคัญ:
    # อย่าเอา background ไปเขียนทับ color_t
    # ถ้ามี color_t จาก HAR ให้เก็บของเดิมไว้
    if item.get('color_t'):
