...
def infinite_local_url(value):
    ...
    """
    เปลี่ยน URL รูป JOYPOP
    ให้ใช้รูปที่ Mirror อยู่ในเครื่อง
    """
...
def infinite_local_file_exists(url):
    ...
    """
    ตรวจว่ารูปที่ Mirror
    มีอยู่จริงใน img.joypop.gg/
    """
...
def infinite_find_product_image(item):
    ...
    imgurltwo เอาไว้ท้ายสุด
    เพราะบางตู้ใช้เป็น Overlay
    """
...
def infinite_find_background(gid, level, item):
    ...
    # 1) back_image จริงของรางวัล
    bg=item.get('back_image')

    if bg:
        return infinite_local_url(bg)

    # 2) background field อื่น
    for key in (
        'background_image',
        'background',
        'goods_back_image',
    ...
    # 3) หาใน Tier เดียวกันของตู้
    for source in pool.get(level,[]) or []:
    ...
    # 4) color_t เป็น fallback เท่านั้น
    # ห้ามเอามาก่อน back_image
    bg=item.get('color_t')
