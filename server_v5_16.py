...
def infinite_local_url(value):
    """Normalize JOYPOP image URLs to the local mirror without locking port 8080."""
    if not value:
        return ''
    ...
    if value.startswith(('uploads/','upload/','static/','newimage/')):
...
def infinite_find_product_image(item):
    candidates=[]
    for key in (
        'goodslist_imgurl','imgurl','image','content_image','goods_imgurl',
...
def infinite_find_background(gid, level, item):
    ...
    raw=item.get('color_t')
    if raw:
        return infinite_local_url(raw)
    for source in tier_items:
        raw=source.get('color_t')
        if raw:
            return infinite_local_url(raw)

    fallback={
        'SP':'/local-img/static/phase-two/SP.png',
        'A':'/local-img/static/phase-two/A.png',
        'B':'/local-img/static/phase-two/B.png',
        'C':'/local-img/static/phase-two/C.png',
        'D':'/local-img/static/phase-two/D.png'
    }.get(level,'')
    return fallback if fallback and infinite_local_file_exists(fallback) else ''


def apply_infinite_presentation(gid, item):
    gid=str(gid)
    level=str(item.get('shang_title') or '').strip().upper()

    image=infinite_find_product_image(item)
    if image:
        for key in ('goodslist_imgurl','imgurl','content_image','image','goods_image','prize_image'):
            item[key]=image
    else:
        print('[V5.16 IMAGE MISSING]', 'gid=',gid,'tier=',level,
              'title=',item.get('goodslist_title') or item.get('title') or '',
              'original=',item.get('goodslist_imgurl') or item.get('imgurl') or item.get('image') or '')

    background=infinite_find_background(gid,level,item)
    if background:
        for key in ('back_image','background_image','background','goods_back_image',
                    'goods_background','result_background','result_back_image'):
            item[key]=background
    else:
        print('[V5.16 BACKGROUND MISSING]','gid=',gid,'tier=',level)

    if item.get('color_t'):
        item['color_t']=infinite_local_url(item['color_t'])

    badge=item.get('shang_image') or LEVEL_BADGES.get(level,'')
    if badge:
        item['shang_image']=infinite_local_url(badge)
