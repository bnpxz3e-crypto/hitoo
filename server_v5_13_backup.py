...
def local_card_shang_logs(p):
    """History tab: only packs opened by this local user."""
    ...
    prof=load_profile(); hist=list(reversed(load_stats().get('history',[])))
    # Hide legacy B/C/D-only rows that may already exist in local_stats.json.
    # All = SP or A only; SP/A tabs then apply their own filter.
    hist=[h for h in hist if str(h.get('goods_id'))==gid and (h.get('has_sp') or h.get('has_a'))]
    if wanted: hist=[h for h in hist if h.get('has_'+wanted.lower())]
    rows=[]
    for h in hist:
        cards=h.get('cards') or []
        if not cards: continue
        # For SP/A tabs, lead with the matching card; All leads with highest tier.
        order={'SP':0,'A':1,'B':2,'C':3,'D':4}
        cards=sorted(cards,key=lambda x:order.get(str(x.get('tier') or '').upper(),9))
        if wanted:
            cards=sorted(cards,key=lambda x:0 if str(x.get('tier') or '').upper()==wanted else 1)
        lead=cards[0]; ts=h.get('time') or ''
        try: addunix=int(time.mktime(time.strptime(ts,'%Y-%m-%d %H:%M:%S')))
        except: addunix=int(time.time())
        goods=[]
        for i,c in enumerate(cards,1):
            tier=str(c.get('tier') or '').upper(); img=c.get('image') or ''
            goods.append({'id':h.get('order_interval_num',0)+i,'sale_num':h.get('order_interval_num',0),'order_interval_num':h.get('order_interval_num'),
              'goodslist_imgurl':img,'shang_id':{'SP':100,'A':101,'B':102,'C':103,'D':104}.get(tier,104),'num':1,
              'goodslist_money':str(c.get('price') or '0'),'goodslist_price':str(c.get('price') or '0'),'prize_type':1,
              'goodslist_id':0,'real_goods_list_id':0,'goodslist_title':c.get('title') or '', 'short_title':c.get('title') or '',
              'shang_title':tier,'shang_image':LEVEL_BADGES.get(tier,'')})
        tier=str(lead.get('tier') or h.get('tier') or '').upper()
        rows.append({'id':h.get('order_interval_num'),'user_id':999999,'shang_id':{'SP':100,'A':101,'B':102,'C':103,'D':104}.get(tier,104),
          'goodslist_id':0,'real_goods_list_id':0,'goodslist_imgurl':lead.get('image') or '', 'sale_num':h.get('order_interval_num'),
          'order_interval_num':h.get('order_interval_num'),'order_id':h.get('order_interval_num'),'addtime':addunix,
          'goodslist_price':str(lead.get('price') or '0'),'interval_num':1,'goodslist_title':lead.get('title') or '',
          'short_title':lead.get('title') or '', 'price':str(lead.get('price') or '0'),'goodslist_money':str(lead.get('price') or '0'),
          'shang_title':tier,'shang_image':LEVEL_BADGES.get(tier,''),'straddtime':0,'nickname':prof['name'],'headimg':'','head_border':'',
          'sum_price':sum(float(c.get('price') or 0) for c in cards),'goods_imgurl':'','goodslist':goods})
    total=len(rows); start=(page-1)*limit; rows=rows[start:start+limit]
    rr=effective_rates(gid,CARD_DETAILS.get(gid,{}).get('data',{}))
    cats=[{'shang_id':0,'shang_title':'ทั้งหมด'}]
    for lvl,sid in [('SP',100),('A',101)]:
        if lvl in rr: cats.append({'shang_id':sid,'sum_real_pro':f"{float(rr[lvl]):.8f}",'max_sort':0,'shanginfo':{'id':sid,'title':lvl,'shang_level':4 if lvl=='SP' else 3,'image':LEVEL_BADGES.get(lvl,'')},'lilun':round(100/float(rr[lvl])) if float(rr[lvl]) else 0,'shang_title':lvl})
    return {'status':1,'msg':'Request successful','data':{'interval_num_average':1,'sh_no':0,'category':cats,'data':rows,'total':total,'current_page':page,'last_page':max(1,(total+limit-1)//limit)}}
...
print(f'JOYPOP Local V5.27 -> 0.0.0.0:{PORT}')
